  ...
  return (
    <div
      ...
      <Suspense fallback={< div > Загрузка ...</div>}>
        ...
        {cartStore.length
          ? cartStore.map(item => (<CartItem  selected={(id) => selected(id)} unSelected={(id) => unSelected(id)} data={item} key={item._id}/>))
          : <h1 className="w-100">There is nothing in the basket</h1>
        }
      </Suspense>
    </div>
  );
